  ...
  if (normalized.length >= 6) {
    ...
    if (interruptionIndex >= 0) {
      ...
      for (let index = 2; index < normalized.length - 3; index += 1) {
        ...
        if (flankInternalVariation <= 0.08 && Math.abs(leftFlankMean - rightFlankMean) <= 0.1 && segmentVariation <= 0.06 && segmentMean >= 0.3 && segmentDip >= 0.1 && segmentDip <= 0.28) {
...
const coherentSoftSegment = coherence([132, 136, 92, 96, 133, 137]);
...
if (!(coherentSoftSegment > broadMissingSegment)) throw new Error(`A coherent soft edge segment must retain more support than a genuinely absent segment: soft=${coherentSoftSegment}, absent=${broadMissingSegment}`);
if (!(coherentSoftSegment > unstableSoftSegment)) throw new Error(`Soft-segment relief must require coherent flanks: soft=${coherentSoftSegment}, unstable=${unstableSoftSegment}`);
if (!(isolatedGlare >= coherentSoftSegment)) throw new Error(`A single glare interruption may receive at least as much support as a two-sample soft segment: glare=${isolatedGlare}, soft=${coherentSoftSegment}`);
...
if (!(coherentSoftSegment > repeatedOscillatingReflection)) throw new Error(`A coherent soft segment must retain more support than repeated oscillation: soft=${coherentSoftSegment}, noise=${repeatedOscillatingReflection}`);
...
console.log("Segment-aware strength-coherent jitter smoke passed: coherent soft edge sections retain bounded support while absent or unstable gaps stay suppressed.");
