...
if (missing.length) throw new Error(`Contrast-retention-aware strength-coherent jitter patch incomplete: ${JSON.stringify(missing)}`);
  ...
  if (normalized.length >= 6) {
    ...
    if (interruptionIndex >= 0) {
      ...
      for (let index = 2; index < normalized.length - 3; index += 1) {
        ...
        const segmentContrastRetention = segmentMean / Math.max(flankMean, 0.001);
        if (flankInternalVariation <= 0.08 && Math.abs(leftFlankMean - rightFlankMean) <= 0.1 && segmentVariation <= 0.06 && flankMean >= 0.4 && segmentContrastRetention >= 0.62 && segmentDip >= 0.1 && segmentDip <= 0.28) {
...
if (!(coherentShadowSegment > lowContrastReflectionSegment)) throw new Error(`A shadowed edge retaining local contrast must outrank a similarly shaped low-contrast reflection: shadow=${coherentShadowSegment}, reflection=${lowContrastReflectionSegment}`);
...
if (!(coherentShadowSegment > unstableSoftSegment)) throw new Error(`Contrast-aware soft-segment relief must require coherent flanks: shadow=${coherentShadowSegment}, unstable=${unstableSoftSegment}`);
...
console.log("Contrast-retention-aware strength smoke passed: coherent shadowed edges retain support while low-contrast reflections and absent gaps stay suppressed.");