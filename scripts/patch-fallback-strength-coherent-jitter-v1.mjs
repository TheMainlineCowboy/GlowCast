...
if (
  ...
  !source.includes("bilateralContrastRetention >= 0.62") ||
  !source.includes("Math.min(leftContrastRetention, rightContrastRetention)") ||
  !source.includes("secondaryGapGradientDirectionalAllowance * secondaryGapGradientStrengthCoherence")
) {
  throw new Error("Bilateral-contrast-aware strength-coherent sparse jitter resistance was not applied.");
}

await fs.writeFile(path, source);
console.log("Preserved coherent soft edge segments only when both sides retain architectural contrast.");
