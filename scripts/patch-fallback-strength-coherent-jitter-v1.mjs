...
if (
  ...
  !source.includes("flankInternalVariation <= 0.08") ||
  !source.includes("flankAgreement <= 0.1") ||
  !source.includes("secondaryGapGradientDirectionalAllowance * secondaryGapGradientStrengthCoherence")
) {
  throw new Error("Flank-validated strength-coherent sparse jitter resistance was not applied.");
}

await fs.writeFile(path, source);
console.log("Bounded glare relief to one isolated sample surrounded by coherent edge flanks.");
