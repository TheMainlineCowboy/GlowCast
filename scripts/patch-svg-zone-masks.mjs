...
const helperMarker = "function renderProjectionLayer(extra = \"\") {";
const helper = `function svgMaskShape(zone: ProjectZone, key: string) {
...
if (!source.includes("function svgMaskShape(zone: ProjectZone, key: string)")) {
...
source = source.replace(
  ...
  `{includedZones.map((zone) => svgMaskShape(zone, \`pm-${zone.id}\`))}`
);
