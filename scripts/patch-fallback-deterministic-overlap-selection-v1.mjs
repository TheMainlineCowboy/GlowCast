...
if (source.includes(marker) && source.includes(coverageMarker) && !source.includes(oldSelection)) {
  console.log("Continuous-coverage nested fallback overlap selection already applied.");
  process.exit(0);
}

if (source.includes(marker) && source.includes(spreadMarker) && !source.includes(oldSelection)) {
  const currentSpreadSelection = `    // Resolve overlapping established masks deterministically. Stronger overlap wins;
  ...
  if (!source.includes(currentSpreadSelection)) {
    throw new Error("Existing perimeter-spread fallback selection block was not recognized.");
  }
  source = source.replace(currentSpreadSelection, buildSelectionBlock());
} else if (source.includes(marker) && source.includes(qualityMarker) && !source.includes(oldSelection)) {
  throw new Error("Perimeter-quality fallback selection exists without the expected spread ranking.");
} else if (source.includes(marker) && !source.includes(oldSelection)) {
  throw new Error("Deterministic fallback selection exists without the expected quality ranking.");
} else {
...
if (!source.includes(marker) || !source.includes(qualityMarker) || !source.includes(spreadMarker) || !source.includes(coverageMarker) || source.includes(oldSelection)) {
  throw new Error("Continuous-coverage nested fallback overlap selection was not applied.");
}

await fs.writeFile(path, source);
console.log("Selected nested fallback overlap targets by overlap, perimeter completeness, continuous side coverage, spread, established surface size, and stable order.");
