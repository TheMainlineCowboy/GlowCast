...
const marker = "const balancedFallbackExpansion = horizontalExpansionImbalance <= 0.28 && verticalExpansionImbalance <= 0.28;";
if (source.includes(marker)) {
  console.log("Fallback duplicate balanced-expansion gate already applied.");
  process.exit(0);
}

const anchor = `      const fallbackGrowthRatio = fallbackArea / Math.max(existingArea, 1);
...
const replacement = `      const fallbackGrowthRatio = fallbackArea / Math.max(existingArea, 1);
      const boundedFallbackGrowth = fallbackGrowthRatio <= 1.8;
...
if (!source.includes(anchor)) {
  throw new Error("Fallback duplicate balanced-expansion anchor not found.");
}

source = source.replace(anchor, replacement);

if (!source.includes(marker) || !source.includes("balancedFallbackExpansion &&")) {
  throw new Error("Fallback duplicate balanced-expansion gate was not applied.");
}

await fs.writeFile(path, source);
console.log("Prevented one-sided trim or seam growth from inflating a stronger architectural mask.");
