...
if (!source.includes('Review Auto Masks Only')) {
  const previewButtonPattern = /^(\s*)<button className="primary" onClick=\{\(\) => \{ setProjectionOnly\(\(value\) => !value\); \}\} disabled=\{!hasProject\} >/m;
  const match = source.match(previewButtonPattern);
  if (!match) throw new Error("Mask review button anchor not found.");
  source = source.replace(previewButtonPattern, filterButton + match[0]);
}
