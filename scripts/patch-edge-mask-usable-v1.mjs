...
writeFileSync("src/core/edgeMaskCandidates.ts", `import type { EdgePoint } from "../edgeDetect";
...
function uniqueCandidates(candidates: EdgeMaskCandidate[], limit: number) {
  const sorted = candidates
    .filter((c) => c.width > 1.8 && c.height > 1.8)
...
function makeCandidate(minX: number, maxX: number, minY: number, maxY: number, cellSize: number, count: number): EdgeMaskCandidate | null {
  ...
  if (count < 3 || width < 1.8 || height < 1.8 || area < 6 || area > 1400 || density < 0.025) return null;
  return {
...
function clusterCandidates(edgePoints: EdgePoint[], limit: number): EdgeMaskCandidate[] {
  ...
  const dynamicThreshold = Math.max(50, strengths[Math.floor(strengths.length * 0.50)] ?? 50);
  for (const point of edgePoints) {
    if (point.strength < dynamicThreshold) continue;
    occupied.add(String(Math.floor(point.x / cellSize)) + "," + String(Math.floor(point.y / cellSize)));
  }
  ...
  for (const key of occupied) {
    ...
    while (queue.length) {
      ...
      for (const [ox, oy] of neighbors) {
        const next = String(x + ox) + "," + String(y + oy);
