...
try {
  ...
  const centeredEdges = [];
  addFrame(centeredEdges, 16, 16, 44, 44);
  ...
  if (shiftedRight.length !== 1) {
    throw new Error(`Right-shifted fallback created a duplicate: ${JSON.stringify(shiftedRight)}`);
  }
  ...
  if (shiftedDown.length !== 1) {
    throw new Error(`Down-shifted fallback created a duplicate: ${JSON.stringify(shiftedDown)}`);
  }
