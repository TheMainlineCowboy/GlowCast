...
const distributionMarker = "const secondaryClusterDistribution = dominantGapCandidate";
if (source.includes(marker) && source.includes(consistentSpacingMarker) && source.includes(distributionMarker)) {
  console.log("Spatially distributed cluster-weighted corner-pair confidence ranking already applied.");
  process.exit(0);
}
...
if (
  ...
  !source.includes(distributionMarker) ||
  !source.includes("const orderedGaps = bestRun.slice(1)") ||
  !source.includes("const secondaryGapIndices = orderedGaps") ||
  !source.includes("Math.sqrt(secondaryClusterDistribution)") ||
  ...
  throw new Error("Spatially distributed cluster-weighted corner-pair confidence strength ranking was not applied.");
}

await fs.writeFile(path, source);
console.log("Ranked nested perimeter confidence using sample support and spatial distribution of local edge-spacing clusters.");