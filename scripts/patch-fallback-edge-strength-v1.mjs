...
const stableSpacingMarker = "const stableGaps = gaps.slice(0, Math.max(1, Math.ceil(gaps.length * 0.6)));";
if (source.includes(marker) && source.includes(stableSpacingMarker)) {
  console.log("Stable-cluster corner-pair confidence ranking already applied.");
  process.exit(0);
}
...
if (
  ...
  !source.includes(stableSpacingMarker) ||
  ...
  throw new Error("Stable-cluster corner-pair confidence strength ranking was not applied.");
}

await fs.writeFile(path, source);
console.log("Ranked nested perimeter confidence using corner tolerances derived from stable local edge-spacing clusters and image scale.");
