...
const sampleConfidenceMarker = "const secondaryClusterAuthority = dominantGapCandidate";
if (source.includes(marker) && source.includes(consistentSpacingMarker) && source.includes(sampleConfidenceMarker)) {
  console.log("Sample-weighted corner-pair confidence ranking already applied.");
  process.exit(0);
}
...
if (
  ...
  !source.includes("upperCount: upper.length") ||
  !source.includes("const denseGaps = dominantGapCandidate ? gaps.slice(0, dominantGapCandidate.cutoff) : gaps;") ||
  !source.includes("dominantGapCandidate.upperCount / Math.max(dominantGapCandidate.lowerCount, 1)") ||
  !source.includes("const unsplitSpacing = gaps[Math.floor(gaps.length * 0.75)]") ||
  !source.includes("unsplitSpacing * (1 - secondaryClusterAuthority) + denseMedianSpacing * secondaryClusterAuthority") ||
  ...
  throw new Error("Sample-weighted corner-pair confidence strength ranking was not applied.");
}

await fs.writeFile(path, source);
console.log("Ranked nested perimeter confidence using sample-weighted, internally consistent local edge-spacing clusters and image scale.");
