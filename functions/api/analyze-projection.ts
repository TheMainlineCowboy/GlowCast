import {DETECTOR_MODEL,DETECTOR_PROMPT,Env,SAM_MODEL,dets,masks,path,rect,run,type Mask,type Point,type Polygon} from "./cv-core";
...
async function analyze(imageDataUrl:string,env:Env,refinement?:{positivePoints?:Point[];negativePoints?:Point[];maskInsetOutsetPx?:number}){
  ...
  if(!foundMasks.length)warnings.push("No usable masks returned; frontend may use fallback manual masks.");
