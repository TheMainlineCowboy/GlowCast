...
// Based on src/shapeGeometry.ts

export type GeometryPoint = { x: number; y: number; };

export type MaskShape = "rectangle" | "circle" | "oval" | "triangle" | "freehand";
...
/**
 * UPDATED: Now creates ledges using Geometry Points
 * This allows snow to land on the curves of circles/ovals.
 */
function createLedgesFromZones(zones: ProjectZone[], canvasWidth: number, canvasHeight: number): SnowLedge[] {
  const ledges: SnowLedge[] = [];

  zones.filter(z => z.included).forEach(z => {
    // Get mathematical points for the shape
    const points = zoneToGeometryPoints(z).map(p => ({
      x: (p.x / 100) * canvasWidth,
      y: (p.y / 100) * canvasHeight
    }));

    // Create a ledge for every "top-facing" segment of the shape
    for (let i = 0; i < points.length; i++) {
      ...
      // Only treat segments that are roughly horizontal-ish as ledges
      if (Math.abs(dy / dx) < 1.4 && p1.x !== p2.x) {
        ...
        ledges.push({
          ...
          normalX: -dy / len,
          normalY: dx / len,
          accumulation: new Array(Math.floor(xMax - xMin)).fill(0)
        });
      }
    }
  });
...
export default function App() {
  ...
  function handlePointerMove(event: React.PointerEvent) {
    const point = resizeAction ? getPoint(event, false) : getPoint(event); 
    if (!point) return;
