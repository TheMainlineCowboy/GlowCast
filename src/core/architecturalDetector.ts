...
function nearSurfaceEdge(x: number, y: number, width: number, height: number, surface: Bounds) {
  ...
  return x <= surface.x + mx || y <= surface.y + my || x + width >= surface.x + surface.width - mx || y + height >= surface.y + surface.height - my;
}
...
function boxToCandidate(box: ComponentBox, allPoints: EdgePoint[], lines: LineSegment[], surface: Bounds, id: string): CandidateProposal | null {
  ...
  const skinnyEdgeArtifact = aspect < 0.42 && nearSurfaceEdge(x, y, width, height, surface);
...
function linePairCandidates(points: EdgePoint[], lines: LineSegment[], surface: Bounds) {
  ...
  for (const left of vertical) {
    for (const right of vertical) {
      ...
      if (doorLike && nearSurfaceEdge(x, y, width, height, surface)) continue;
    ...
    for (let x0 = 0; x0 < cols; x0++) {
      for (let w = 2; w <= 6; w++) {
        for (let h = 2; h <= 7; h++) {
          ...
          candidates.push({ id: `grid-fallback-${id++}`, x: Number(x.toFixed(2)), y: Number(y.toFixed(2)), width: Number(width.toFixed(2)), height: Number(height.toFixed(2)), score, contributingLines: hLines + vLines, status: score >= 70 ? "high" : "low" });
        }
      }
    }
  }
